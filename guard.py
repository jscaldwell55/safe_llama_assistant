...
class EnhancedGuardAgent:
    ...
    def _check_violations_by_intent(
        ...
        if intent == "CONVERSATIONAL_BRIDGE":
            ...
            # Clarifying questions don't need grounding
            # Only check for inappropriate medical directives
            if self._contains_medical_directive(response):
                violations.append({
                    "type": "medical_advice",
                    "severity": "critical",
    ...
    def _validate_factual_response(
        ...
        # Skip grounding check if no context was provided
        if not context or context.strip() == "[NO CONTEXT PROVIDED]":
            # For responses without context, only check for medical safety
            medical_violations = self._check_medical_safety(response, "")
    ...
    def _extract_factual_claims(self, response: str) -> List[Dict[str, Any]]:
        ...
        for sentence in sentences:
            ...
            # Check if conversational or gap acknowledgment
